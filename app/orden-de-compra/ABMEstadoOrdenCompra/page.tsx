...
export default function ABMEstadoOrdenPage() {
  ...
  const fetchEstados = async () => {
    try {
      ...
      const data: EstadoOrdenCompra[] = raw.map((r) => ({
        id: r.idEOC,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/ABMEstadoOrdenCompra/bajaEstado?idEstadoOrdenCompra=${selectedEstado.id}`,
        {
          method: "POST",
        }
      );
      ...
      if (response.ok) {
        alert(
          `Estado "${selectedEstado.nombreEstado}" dado de baja exitosamente`
        );
        ...
        alert(
          `No se puede dar de baja el estado "${selectedEstado.nombreEstado}".\n\n` +
            `El estado puede estar siendo utilizado por órdenes de compra existentes.`
        );
      }
    } catch (error) {
      console.error("Error al dar de baja estado:", error);
      alert(
        `Error al dar de baja el estado: ${
  ...
  const estadosActivos = estados.filter(
    (estado) => estado.fechaHoraBaja === null
  ).length;
  const estadosDadosBaja = estados.filter(
    (estado) => estado.fechaHoraBaja !== null
  ).length;
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-slate-400">
              Cargando estados de orden de compra...
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-1 flex-col gap-4 p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400">
            Administrar estados de órdenes de compra
          </p>
        </div>

        <Link href="/orden-de-compra/ABMEstadoOrdenCompra/NuevoEstado">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Checkbox
          ...
          onCheckedChange={() => setShowOnlyActive((prev) => !prev)}
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-2">
                          <Link
                            href={`/orden-de-compra/ABMEstadoOrdenCompra/ModificarEstado/${estado.id}`}
                          >
                            <Button
                              size="sm"
                              variant="outline"
        ...
        <DialogContent className="bg-slate-800 border-slate-700 text-slate-100">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-300">
              ¿Está seguro que desea dar de baja el siguiente estado de orden de
              compra?
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="py-4">
              ...
              <div className="mt-4 p-3 bg-yellow-900/20 border border-yellow-600/30 rounded-lg">
                <p className="text-yellow-200 text-sm">
                  <strong>Nota:</strong> No se podrá dar de baja si el estado
                  está siendo utilizado por órdenes de compra existentes.
                </p>
              </div>
            </div>
          )}
