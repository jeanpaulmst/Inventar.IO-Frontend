...
interface DTODetallesOCEnviar {
  ...
  costoPedido: number;
  proveedorPredeterminado: boolean;
  nombreArt: string;
...
export default function EnviarOrdenCompraPage() {
  ...
  const handleEnviar = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL_ENVIAR}/confirmar?idOC=${orden.idOC}`,
        {
          method: "POST",
          headers: {
  ...
  // Calcular totales
  const totalOrden =
    orden?.detallesOC.reduce((total, detalle) => total + detalle.subTotal, 0) ||
    0;
  const totalCostoPedido =
    orden?.detallesOC.reduce(
      (total, detalle) => total + detalle.costoPedido,
      0
    ) || 0;
  const totalCostoAlmacenamiento =
    orden?.detallesOC.reduce(
      (total, detalle) => total + detalle.costoAlmacenamientoArt,
      0
    ) || 0;
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-700 hover:bg-slate-700 border-slate-600">
                      <TableHead className="text-green-200 font-semibold">
                        Artículo
                      </TableHead>
                      <TableHead className="text-green-200 font-semibold">
                        Proveedor
                    ...
                    {orden.detallesOC.map((detalle, index) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            className={`${
                              detalle.proveedorPredeterminado
                                ? "bg-green-600 text-white"
                                : "bg-slate-600 text-slate-300"
                            }`}
                          >
                            {detalle.proveedorPredeterminado ? "Sí" : "No"}
                          </Badge>
                        </TableCell>
                      </TableRow>
                    ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="p-4 bg-slate-700 rounded-lg border border-slate-600">
                ...
                <span className="text-orange-400 font-bold text-xl">
                  ${totalCostoPedido.toFixed(2)}
                </span>
              </div>
              <div className="p-4 bg-slate-700 rounded-lg border border-slate-600">
                <h4 className="text-slate-200 font-medium mb-2">
                  Total Costo Almacenamiento
                </h4>
                <span className="text-blue-400 font-bold text-xl">
                  ${totalCostoAlmacenamiento.toFixed(2)}
                </span>
              </div>
            </div>
