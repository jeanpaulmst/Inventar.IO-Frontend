...
export default function ListarArticuloProveedorPage() {
  ...
  const [editingArticuloProveedor, setEditingArticuloProveedor] =
    useState<ArticuloProveedor | null>(null);
  ...
  const fetchProveedores = async () => {
    try {
      const response = await fetch(
        `${API_URL}/ABMProveedor/getProveedores?soloVigentes=0`
      );
      if (response.ok) {
        const data = await response.json();
        setProveedores(data || []);
  ...
  const handleEdit = (articuloProveedor: ArticuloProveedor) => {
    ...
    const dto = {
      ...
      predeterminado: articuloProveedor.predeterminado,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      setIsDeleting(true);
      const response = await fetch(
        `${API_URL}/asignarProveedor/eliminar/${id}`,
        {
          method: "PUT",
        }
      );
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-slate-400">
            Gestión de asignaciones de artículos a proveedores
          </p>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-750 p-4 rounded-lg">
                <h4 className="text-slate-200 font-semibold mb-2">
                  Artículo: {editingArticuloProveedor.articulo.nombre}
                </h4>
                <p className="text-slate-400 text-sm">
                  ID: {editingArticuloProveedor.articulo.id}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="modeloInventario" className="text-slate-300">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setEditForm({
                        ...editForm,
                        modeloInventarioId: parseInt(value),
                      })
                    }
                  >
                    <SelectTrigger
                ...
                <div className="space-y-2">
                  <Label htmlFor="costoPedido" className="text-slate-300">
                    Costo de Pedido
                  </Label>
                  <Input
                    id="costoPedido"
                ...
                <div className="space-y-2">
                  <Label htmlFor="costoUnitario" className="text-slate-300">
                    Costo Unitario
                  </Label>
                  <Input
                    id="costoUnitario"
                ...
                <div className="space-y-2">
                  <Label htmlFor="demoraEntrega" className="text-slate-300">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        demoraEntrega: parseInt(e.target.value) || 0,
                      })
                    }
                    className="bg-slate-700 border-slate-600 text-slate-100"
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="nivelServicio" className="text-slate-300">
                    Nivel de Servicio (%)
                  </Label>
                  <Input
                    id="nivelServicio"
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        nivelServicio: parseFloat(e.target.value) || 0,
                      })
                    }
                    className="bg-slate-700 border-slate-600 text-slate-100"
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="stockSeguridad" className="text-slate-300">
                    Stock de Seguridad
                  </Label>
                  <Input
                    id="stockSeguridad"
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        stockSeguridad: parseInt(e.target.value) || 0,
                      })
                    }
                    className="bg-slate-700 border-slate-600 text-slate-100"
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="predeterminado" className="text-slate-300">
                    Predeterminado
                  </Label>
                  <div className="flex items-center space-x-2">
                    <Switch
                      id="predeterminado"
                      checked={editForm.predeterminado}
                      onCheckedChange={(checked) =>
                        setEditForm({ ...editForm, predeterminado: checked })
                      }
                    />
                    <span className="text-slate-300 text-sm">
                      {editForm.predeterminado ? "Sí" : "No"}
                    </span>
                  </div>
                </div>

                <div className="space-y-2">
                  <Label htmlFor="tiempoFijo" className="text-slate-300">
                    Tiempo Fijo
                  </Label>
                  <Input
                    id="tiempoFijo"
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        tiempoFijo: parseInt(e.target.value) || 0,
                      })
                    }
                    className="bg-slate-700 border-slate-600 text-slate-100"
                  />
                ...
                <div className="space-y-2">
                  <Label htmlFor="proximaRevision" className="text-slate-300">
                    Próxima Revisión
                  </Label>
                  <Input
                    id="proximaRevision"
                    type="datetime-local"
                    value={
                      editForm.proximaRevision
                        ? new Date(editForm.proximaRevision)
                            .toISOString()
                            .slice(0, 16)
                        : ""
                    }
