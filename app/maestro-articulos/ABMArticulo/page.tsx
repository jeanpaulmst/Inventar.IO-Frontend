...
export default function ABMArticuloPage() {
  ...
  const fetchArticulos = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/ABMArticulo/getAll?soloVigentes=false`
      );
  ...
  const handleProveedor = (articuloId: number) => {
    router.push(
      `/maestro-articulos/ABMArticulo/AsignarProveedor/${articuloId}`
    );
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/ABMArticulo/baja/${selectedArticulo.id}`,
        {
          method: "PUT",
        }
      );
      ...
      if (response.ok) {
        alert(
          `Artículo "${selectedArticulo.nombre}" dado de baja exitosamente`
        );
      ...
      alert(
        `Error al dar de baja el artículo: ${
  ...
  const articulosActivos = articulos.filter(
    (art) => art.fhBajaArticulo === null
  ).length;
  const articulosDadosBaja = articulos.filter(
    (art) => art.fhBajaArticulo !== null
  ).length;
  const valorTotalInventario = articulos.reduce(
    (total, art) => total + art.precioUnitario * art.stock,
    0
  );
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-700 hover:bg-slate-700 border-slate-600">
                  ...
                  <TableHead className="text-blue-200 font-semibold text-center">
                    Acciones
                  </TableHead>
                </TableRow>
              </TableHeader>
                  ...
                  articulos.map((articulo, index) => (
                    ...
                    >
                      ...
                      <TableCell
                        ...
                        title={articulo.descripcionArt}
                      >
                        {articulo.descripcionArt}
                      </TableCell>
                      <TableCell
                        className={`${
                          articulo.fhBajaArticulo
                            ? "text-red-400"
                            : "text-green-400"
                        }`}
                      >
                      ...
                      <TableCell>
                        {articulo.proveedorPredeterminado ? (
                          ...
                          <Button
                            ...
                            title={
                              articulo.fhBajaArticulo !== null
                                ? "No se puede asignar proveedor a un artículo dado de baja"
                                : ""
                            }
                          >
                      ...
                      <TableCell>
                        <div className="flex items-center justify-center gap-2">
                          <Button
                            ...
                            title={
                              articulo.fhBajaArticulo !== null
                                ? "No se puede modificar un artículo dado de baja"
                                : ""
                            }
                          >
                            <Edit className="w-3 h-3" />
